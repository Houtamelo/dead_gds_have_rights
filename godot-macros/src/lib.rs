...
///
/// ### Editor reload and instance state
/// Editor reload ("hot reload") reconstructs your Rust instances from scratch, which can sometimes be surprising. This is especially
/// relevant for `#[class(tool)]` classes, whose live instances run in the editor and are therefore reconstructed on every reload. (Non-tool
/// classes are represented by engine-side placeholders in the editor, but the same property save/restore described below applies to them.)
///
/// When the editor reloads an extension, Godot:
/// 1. saves the properties whose usage flags include `STORAGE`;
/// 2. destroys the old Rust instance;
/// 3. constructs a fresh one (your `init()` runs again);
/// 4. patches it back into the scene tree and restores the saved properties via their setters.
///
/// Crucially, Godot does **not** re-fire `ready()` or `enter_tree()` -- those are one-shot tree-entry hooks. But technically your nodes stay
/// in the tree across reload, it's just the Rust part of the Godot object that's recreated. So anything you set up inside `ready()` --
/// assigning fields, resolving [`OnReady`](../obj/struct.OnReady.html), etc. is gone after a reload, and the engine will not redo it.
///
/// Two consequences follow:
/// - **`OnReady` fields go back to their uninitialized state.** Accessing them after a reload panics. You can re-resolve them in response to the
///   `EXTENSION_RELOADED` notification, or back them by a `STORAGE`-flagged field (see next point). Read also the
///   [`OnReady`](../obj/struct.OnReady.html#editor-reload) semantics.
/// - **Plain `#[var]` fields are not restored**, because `#[var]` does not include `STORAGE` in its usage flags, so step 1 skips them.
///   `#[export]` does include `STORAGE`. To persist a `#[var]` across reloads, opt in explicitly with `#[var(usage_flags = [STORAGE])]`
///   or use `#[export(storage)]`. Note that this serializes your values into the `.tscn` scene file.
///
/// ## Class renaming
